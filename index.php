 ...
 * Search + Present/Absent Lists + Attendance Statistics
 */
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['save_member'])) {
            if (!empty($_POST['member_id'])) {
                $pdo->prepare("UPDATE members SET full_name = ? WHERE id = ?")
                    ->execute([trim($_POST['name']), $_POST['member_id']]);
            } else {
                $pdo->prepare("INSERT INTO members(full_name) VALUES (?) ON CONFLICT DO NOTHING")
        ...
        if (isset($_POST['mark'])) {
            $pdo->prepare("INSERT INTO attendance(session_id, member_id) VALUES (?, ?) ON CONFLICT DO NOTHING")
                ->execute([$_POST['sid'], $_POST['mid']]);
        }

        if (isset($_POST['set_payment'])) {
            $pdo->prepare("
                INSERT INTO payments (member_id, amount, due_date, status)
                VALUES (?, ?, ?, 'unpaid')
    ...
    if (isset($_GET['action'])) {
        if ($_GET['action'] === 'del_m') {
            $pdo->prepare("DELETE FROM members WHERE id = ?")->execute([$_GET['id']]);
        }

        if ($_GET['action'] === 'del_s') {
            $pdo->prepare("DELETE FROM sessions WHERE id = ?")->execute([$_GET['id']]);
        }

        if ($_GET['action'] === 'unmark') {
            $pdo->prepare("DELETE FROM attendance WHERE member_id = ? AND session_id = ?")
                ->execute([$_GET['mid'], $_GET['sid']]);
        }
    ...
    $total_absent = count($absent_members);
...
<div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
    ...
    <div class="lg:col-span-4 space-y-6">
        ...
        <div class="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl">
            ...
            <nav class="space-y-2">
                <button onclick="toggleView('registry')" id="btn-registry" class="w-full text-left px-6 py-4 rounded-2xl font-bold transition-all tab-active">
                    Athlete Registry
                </button>

                <button onclick="toggleView('sessions')" id="btn-sessions" class="w-full text-left px-6 py-4 rounded-2xl font-bold transition-all text-slate-400 hover:text-white">
                    Sessions
                </button>

                <button onclick="toggleView('export')" id="btn-export" class="w-full text-left px-6 py-4 rounded-2xl font-bold transition-all text-slate-400 hover:text-white">
                    Reports
                </button>
            </nav>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
            ...
            <form method="POST" class="space-y-3">
                ...
                <input
                    ...
                    class="w-full p-4 bg-slate-50 rounded-2xl border-none text-sm ring-1 ring-slate-100 outline-none"
                    required
                >

                <button name="save_member" class="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold shadow-lg">
                    Save Changes
                </button>
            </form>
        </div>
            ...
            <div class="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    <div class="bg-slate-900 text-white rounded-3xl p-5">
                        <p class="text-xs uppercase font-bold text-slate-400">Total Athletes</p>
                        <h3 class="text-3xl font-800"><?= $total_members ?></h3>
                    </div>

                    <div class="bg-emerald-50 text-emerald-700 rounded-3xl p-5">
                        <p class="text-xs uppercase font-bold">Present</p>
                        <h3 class="text-3xl font-800"><?= $total_present ?></h3>
                    </div>

                    <div class="bg-red-50 text-red-600 rounded-3xl p-5">
                        <p class="text-xs uppercase font-bold">Absent</p>
                        <h3 class="text-3xl font-800"><?= $total_absent ?></h3>
                    </div>

                    <div class="bg-indigo-50 text-indigo-700 rounded-3xl p-5">
                        <p class="text-xs uppercase font-bold">Attendance Rate</p>
                        <h3 class="text-3xl font-800"><?= $attendance_rate ?>%</h3>
                    </div>
                </div>
                            ...
                            <?php foreach($members as $m): ?>
                                <tr class="border-b border-slate-50 hover:bg-slate-50/50 athlete-row">
                                    <td class="py-6 athlete-name">
                                        ...
                                        <?php if($m['is_overdue']): ?>
                                            <span class="text-[10px] font-bold text-red-500 uppercase overdue-pulse">
                                                ● Overdue since <?= $m['due_date'] ?>
                                            </span>
                                    ...
                                    <td class="py-6 text-right space-x-2">
                                        <?php if(in_array($m['id'], $attended_ids)): ?>
                                            <a href="?action=unmark&mid=<?= $m['id'] ?>&sid=<?= $current_sid ?>" class="text-[10px] font-900 bg-emerald-100 text-emerald-600 px-3 py-1 rounded-full uppercase">
                                                Present
                                            </a>
                                        <?php else: ?>
                                            <form method="POST" class="inline">
                                                ...
                                                <button name="mark" class="text-indigo-600 font-bold text-xs hover:underline">
                                                    Mark Presence
                                                </button>
                                            </form>
                                        <?php endif; ?>

                                        <button onclick="openPaymentModal(<?= $m['id'] ?>, '<?= addslashes($m['full_name']) ?>')" class="text-slate-400 hover:text-indigo-600 font-bold text-xs">
                                            Set Due
                                        </button>
                                        ...
                                        <?php if($m['status'] === 'unpaid'): ?>
                                            <form method="POST" class="inline">
                                                ...
                                                <button name="pay_now" class="bg-slate-900 text-white px-3 py-1 rounded-lg text-[10px] font-bold">
                                                    Collect $<?= $m['amount'] ?>
                                                </button>
                                            </form>
                                        <?php endif; ?>

                                        <button onclick="editM('<?= $m['id'] ?>','<?= addslashes($m['full_name']) ?>')" class="text-slate-300 hover:text-indigo-500 font-bold text-xs">
                                            Edit
                                        </button>
                                    </td>
                                </tr>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                    ...
                    <div class="bg-emerald-50 rounded-[2rem] p-6">
                        <h3 class="font-800 text-emerald-700 mb-4">
                            Present List
                        </h3>

                        <?php if(count($present_members) > 0): ?>
                            <ul class="space-y-2">
                                <?php foreach($present_members as $pm): ?>
                                    <li class="bg-white rounded-xl px-4 py-3 font-bold text-sm text-slate-700">
                                        <?= htmlspecialchars($pm['full_name']) ?>
                                    </li>
                                <?php endforeach; ?>
                            </ul>
                        <?php else: ?>
                            <p class="text-sm font-bold text-emerald-400">
                                No athletes marked present.
                            </p>
                        <?php endif; ?>
                    </div>

                    <div class="bg-red-50 rounded-[2rem] p-6">
                        <h3 class="font-800 text-red-600 mb-4">
                            Absent List
                        </h3>
            ...
            <div class="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
                ...
                <form method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    <input
                        ...
                        class="p-4 bg-slate-50 rounded-2xl border-none text-sm ring-1 ring-slate-100 outline-none"
                        required
                    >
                    ...
                    <button name="save_session" class="bg-slate-900 text-white rounded-2xl font-bold">
                        Create
                    </button>
                </form>
                ...
                <?php foreach($sessions as $s): ?>
                    <div class="flex justify-between items-center p-4 border-b border-slate-50">
                        ...
                        <a href="?action=del_s&id=<?= $s['id'] ?>" class="text-red-400 text-xs font-bold">
                            Delete
                        </a>
                    </div>
                <?php endforeach; ?>
            ...
            <div class="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
                ...
                <form method="GET" class="flex gap-4">
                    <input
                        ...
                        name="export_date"
                        value="<?= date('Y-m-d') ?>"
                        class="flex-1 p-4 bg-slate-50 rounded-2xl border-none ring-1 ring-slate-100"
                    >

                    <button type="submit" name="export_type" value="all" class="bg-indigo-600 text-white px-8 rounded-2xl font-bold">
                        Download CSV
                    </button>
                </form>
            </div>
...
<div id="pay-modal" class="hidden fixed inset-0 bg-slate-900/60 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-[2.5rem] p-10 w-full max-w-md shadow-2xl">
        ...
        <form method="POST" class="space-y-4">
            ...
            <div>
                <label class="text-[10px] font-bold text-slate-400 uppercase ml-2">
                    Amount Due
                </label>
                ...
                <input
                    ...
                    name="amount"
                    value="50.00"
                    class="w-full p-4 bg-slate-50 rounded-2xl border-none ring-1 ring-slate-100 mt-1 outline-none"
                >
            </div>
            ...
            <button name="set_payment" class="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold shadow-lg">
                Save Deadline
            </button>

            <button type="button" onclick="closeModal()" class="w-full text-slate-400 font-bold text-xs uppercase mt-2 text-center">
                Cancel
            </button>
        </form>
    </div>
...
</html>    } catch (Exception $e) { die("Export Error"); }
}
